...
def test_main_function_runs(mock_model, mock_metrics):
    ...
    with patch('drone_detector_mlops.workflows.test.torch.cuda.is_available', return_value=False), \
...
@pytest.fixture
def simple_model():
    """Simple 2-class classifier for testing."""
    model = nn.Sequential(
        nn.Flatten(),
...
def simple_dataloader():
    """Simple dataloader with known labels."""
    # Create 8 samples: 4 class 0 (drone), 4 class 1 (bird)
