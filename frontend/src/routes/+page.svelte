<script lang="ts">
    ...
    let inferenceTime = $state<number | null>(null);
    ...
    async function analyzeImage() {
        ...
        try {
            const prediction: PredictionResult = await predictImage(selectedFile);

            result = prediction.class_name as ClassificationResult;
        ...
        <div class="w-full max-w-lg">
            ...
            {:else}
                <div class="overflow-hidden rounded-xl bg-redbull-navy-light">
                    ...
                    {#if error}
                        <div class="border-t border-redbull-navy bg-red-900/20 p-6 text-center">
                    ...
                    {#if result}
                        ...
                        >
                            ...
                            {#if confidence !== null}
                                <p class="mt-2 text-sm text-redbull-silver">
                                    Confidence: {(confidence * 100).toFixed(1)}%
                                </p>
                            {/if}
                            {#if inferenceTime !== null}
                                <p class="text-xs text-redbull-silver/60">
